        ...
        for strategy in self.strategies:
            try:
                ...
                if result is not None:
                    logger.info(
                        f"Successfully parsed using strategy: {strategy.__name__}"
                    )
                    return result
...
JSON:"""
    ...
    def _fix_json_formatting(self, text: str) -> str:
        ...
        def fix_unescaped_quotes_in_values(match):
            ...
            value_content = match.group(
                2
            )  # value content that may contain unescaped quotes
        ...
        # Captures everything from key to the final quote of the value. Use DOTALL to allow newlines.
        text = re.sub(
            r'("[^"]+"\s*:\s*")(. *?)"(?=\s*[,}\]])',
            fix_unescaped_quotes_in_values,
            text,
            flags=re.DOTALL,
        )

        # Fix unescaped double quotes in array string values (handles multiline array items)
        def fix_unescaped_quotes_in_array_values(match):
            quote_and_content = match.group(
                1
            )  # " + content that may contain unescaped quotes
            value_content = quote_and_content[1:]  # remove the leading quote

            # Escape any unescaped double quotes in the value content
            escaped_content = re.sub(r'(?<!\\)"', r'\\"', value_content)
            # Escape newlines inside the string value
        ...
        text = re.sub(
            r'(?<=[\[,]\s*)(".*?")(?=\s*[,\]])',
            fix_unescaped_quotes_in_array_values,
            text,
            flags=re.DOTALL,
        )
