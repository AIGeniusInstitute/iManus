...
class DockerSandbox(Sandbox):
    def __init__(
        self, ip: str = None, container_name: str = None, sandbox_session_id: str = None
    ):
        """Initialize Docker sandbox and API interaction client

        Args:
            ip: IP address of sandbox service (used in shared-container mode)
            container_name: Docker container name (used in per-container mode)
            sandbox_session_id: Session identifier inside a shared sandbox container
        """
        ...
        # Session root directory for shared-container mode
        if self._sandbox_session_id:
            self._session_root = f"/home/ubuntu/sessions/{self._sandbox_session_id}"
        else:
    ...
    def id(self) -> str:
        ...
        Returns a unique ID that can be used to retrieve the same sandbox later.
        - Per-container mode: container name
        - Shared-container mode: "shared:{session_id}"
        - Fallback: "dev-sandbox"
        """
        if self._container_name:
            return self._container_name
        if getattr(self, "_sandbox_session_id", None):
            return f"shared:{self._sandbox_session_id}"
        return "dev-sandbox"
    ...
    def _map_path(self, path: str) -> str:
        """Map filesystem paths into per-session root when running in shared-container mode.

        - If sandbox_session is set and path starts with /home/ubuntu, rewrite it to session root.
        - If path is relative, place it under session's upload dir.
        """
        if not getattr(self, "_sandbox_session_id", None):
            return path
        if not path:
            return path
        # Normalize absolute paths under /home/ubuntu
        if path.startswith("/home/ubuntu"):
            # Map /home/ubuntu/... -> /home/ubuntu/sessions/{session_id}/...
            suffix = path[len("/home/ubuntu") :]
            return f"{self._session_root}{suffix}"
        # Relative path -> put under upload dir
        if not path.startswith("/"):
            return f"{self._session_root}/upload/{path}"
        return path
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not tool_result.success:
                    logger.warning(
                        f"Supervisor status check failed: {tool_result.message}"
                    )
                ...
                if all_running:
                    logger.info(
                        f"All {len(services)} services are RUNNING - sandbox is ready"
                    )
                    return  # Success - all services are running
                else:
                    logger.info(
                        f"Waiting for services to start... Non-running: {', '.join(non_running_services)} (attempt {attempt + 1}/{max_retries})"
                    )
                    await asyncio.sleep(retry_interval)

            except Exception as e:
                logger.warning(
                    f"Failed to check supervisor status (attempt {attempt + 1}/{max_retries}): {str(e)}"
                )
                await asyncio.sleep(retry_interval)
    ...
    async def kill_process(self, session_id: str) -> ToolResult:
        response = await self.client.post(
            f"{self.base_url}/api/v1/shell/kill", json={"id": session_id}
        )
        return ToolResult(**response.json())
    ...
    async def file_exists(self, path: str) -> ToolResult:
        ...
        """
        response = await self.client.post(
            f"{self.base_url}/api/v1/file/exists", json={"path": path}
        )
        return ToolResult(**response.json())
    ...
    async def file_delete(self, path: str) -> ToolResult:
        ...
        """
        response = await self.client.post(
            f"{self.base_url}/api/v1/file/delete", json={"path": path}
        )
        return ToolResult(**response.json())
    ...
    async def file_list(self, path: str) -> ToolResult:
        ...
        """
        response = await self.client.post(
            f"{self.base_url}/api/v1/file/list", json={"path": path}
        )
        return ToolResult(**response.json())
    ...
    async def file_upload(
        ...
        response = await self.client.post(
            f"{self.base_url}/api/v1/file/upload", files=files, data=data
        )
        return ToolResult(**response.json())
    ...
    async def file_download(self, path: str) -> BinaryIO:
        ...
        """
        response = await self.client.get(
            f"{self.base_url}/api/v1/file/download", params={"path": path}
        )
        response.raise_for_status()
    ...
    @classmethod
    async def create(cls, logical_session_id: str = None) -> Sandbox:
        """Create a new sandbox instance

        If running in shared-container mode (settings.sandbox_address is set), this will
        create an internal shell session (optionally using the provided logical_session_id)
        and prepare a per-session filesystem root.
        ...
        if settings.sandbox_address:
            ...
            instance = DockerSandbox(ip=ip, sandbox_session_id=logical_session_id)

            # Initialize a per-session shell session inside the shared sandbox container
            try:
                payload = {"exec_dir": None, "command": "echo 'init'"}
                if logical_session_id:
                    payload["id"] = logical_session_id

                response = await instance.client.post(
                    f"{instance.base_url}/api/v1/shell/exec", json=payload
                )
                response.raise_for_status()
                tool_result = ToolResult(**response.json())
                if not tool_result.success:
                    raise Exception(
                        f"Failed to create shared sandbox session: {tool_result.message}"
                    )

                # Get created session id (either server generated or the one we passed)
                session_id = None
                if tool_result.data and isinstance(tool_result.data, dict):
                    session_id = tool_result.data.get("session_id")
                if not session_id and logical_session_id:
                    session_id = logical_session_id
                if not session_id:
                    raise Exception("Failed to obtain sandbox session id")

                instance._sandbox_session_id = session_id
                instance._session_root = f"/home/ubuntu/sessions/{session_id}"

                # Ensure per-session directories exist
                await instance.exec_command(
                    session_id,
                    exec_dir=None,
    ...
    async def get(cls, id: str) -> Sandbox:
        ...
        if settings.sandbox_address:
            ip = await cls._resolve_hostname_to_ip(settings.sandbox_address)
            # Support shared-session ids in the form 'shared:{session_id}'
            if id and id.startswith("shared:"):
                session_id = id.split(":", 1)[1]
                return DockerSandbox(ip=ip, sandbox_session_id=session_id)
            # Fallback: treat id as container name
