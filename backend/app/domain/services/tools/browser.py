...
class BrowserTool(BaseTool):
    ...
    @tool(
        name="browser_wait",
        description="Wait for page load or sleep for specified seconds. Use when waiting for navigation or dynamic content.",
        parameters={
            "timeout": {
                "type": "integer",
    ...
    async def browser_wait(self, timeout: Optional[int] = 15) -> ToolResult:
        """Wait for the page to finish loading or sleep for given seconds

        Args:
            timeout: Maximum wait time in seconds

        Returns:
            ToolResult indicating whether wait succeeded
        """
        # Prefer browser-specific wait_for_page_load if implemented
        try:
            wait_fn = getattr(self.browser, "wait_for_page_load", None)
            if callable(wait_fn):
                success = await wait_fn(timeout)
                return ToolResult(success=bool(success), data={"waited": timeout})
        except Exception as e:
            return ToolResult(success=False, message=str(e))

        # Fallback to asyncio.sleep
        import asyncio

        try:
            await asyncio.sleep(timeout)
            return ToolResult(success=True, data={"waited": timeout})
        except Exception as e:
            return ToolResult(success=False, message=str(e))
