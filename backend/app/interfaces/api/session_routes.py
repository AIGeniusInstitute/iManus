...
@router.post(
    "/{session_id}/clear_unread_message_count", response_model=APIResponse[None]
)
    ...
    async def event_generator() -> AsyncGenerator[ServerSentEvent, None]:
        ...
        while True:
            try:
                ...
                # Only send SSE event when payload changed to avoid noisy repeated messages
...
async def view_shell(
    ...
    result = await agent_service.shell_view(
        session_id, request.session_id, current_user.id
    )
...
async def vnc_websocket(
    ...
    try:
        ...
        await websocket.close(
            code=1011, reason=f"Unable to connect to sandbox environment: {str(e)}"
        )
    except Exception as e:
...
@router.post(
    "/{session_id}/vnc/signed-url", response_model=APIResponse[SignedUrlResponse]
)
async def create_vnc_signed_url(
    ...
    logger.info(
        f"Created signed URL for VNC access for user {current_user.id}, session {session_id}"
    )
